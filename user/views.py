...
def index(request):
    ...
    mydict = {"sd": sdata, "catdata": cdata, "ndata": ndata, "vdata": vdata,
              "cdata": citydata, "cdata1": cdata1, "jobdata": jobdata}
...
def news(request):
    ...
    cid = request.GET.get('x')     # category id
    ctid = request.GET.get('y')    # city id
    ...
    if cid is not None:
        ...
        newsdata = mynews.objects.filter(
            Q(news_headlines__icontains=sdata) | Q(news_description__icontains=sdata)
        )
    else:
...
def viewnews(request):
    ...
    vdata = vnews.objects.none()
    if ndata.exists():
        current_category = ndata.first().news_category
        vdata = vnews.objects.filter(vcategory=current_category).order_by('-id')[0:5]

    md = {"mynews": ndata, "vdata": vdata}
